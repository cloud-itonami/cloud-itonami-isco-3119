(ns engineering.actor
  "EngineeringActor — the ISCO-08 3119 lab support actor as a
  ...
  The unconditional invariant: the EngineeringAdvisor can never
  directly commit a record or dispatch a robot action the
  EngineeringGovernor refuses — every commit-record! call is gated
  behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [engineering.advisor :as advisor]
            [engineering.governor :as governor]
            [engineering.store :as store]))

(defn build-graph
  "Build a compiled EngineeringActor graph. `store` implements
  `engineering.store/Store`. `advisor` implements
  `engineering.advisor/Advisor` (defaults to `mock-advisor`).
